...
public class ClientFeaturesTab implements SlayTab {
    ...
    public ClientFeaturesTab() {
        ...
        buttons.add(new ToggleButton("Squad Overlay", "Share health, armor and cooldowns with your party.", settings::isSquadOverlay, settings::toggleSquadOverlay));
        buttons.add(new ToggleButton("Voice Chat Bridge", "In-game proximity chat with Discord relays.", settings::isVoiceChatBridge, settings::toggleVoiceChatBridge));
        buttons.add(new ToggleButton("Quick Macro Wheel", "Hold V to open a radial for emotes and commands.", settings::isQuickMacroWheel, settings::toggleQuickMacroWheel));
        buttons.add(new ToggleButton("Replay Studio", "Bookmark fights and export cinematic replays.", settings::isReplayStudio, settings::toggleReplayStudio));
        buttons.add(new ToggleButton("Skybox Themes", "Day/night sky swaps that sync with server events.", settings::isSkyboxThemes, settings::toggleSkyboxThemes));
    }
    ...
    public void render(MatrixStack stack, float x, float y, float width, float height, int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.montserrat.drawText(stack, "Flip the switches below to outpace stock LabyMod 3 features.", x, y + 26, ColorUtils.rgba(120, 200, 160, 220), 6, 0.05f);
        float offsetY = y + 40;
